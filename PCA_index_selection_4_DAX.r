...
filenameBasket <- paste("K:\\RANDD\\99-Misc\\Bastiaan\\R-programms\\PCA_index_subbasket_selection\\DAX_basket.csv",sep="")
...
# ####################
    ...
    filename <- paste("C:\\stoxx600_daily_quotes\\DAX Index",".txt",sep="")
...
syncBasketWeights <- numeric(observationsSync)
...
for (i in 1:iNumberOfStocks)
{
    ...
    {
      stock1 <- cIndexNames[[1]][[i]]
      filename <- paste("c:\\syncDAX\\_BOOK_",cIndexNames[[1]][[i]],"_.csv",sep="")      #sync'ed data, synchronized with C#
...
filenameSUBBasket <- paste("K:\\RANDD\\99-Misc\\Bastiaan\\R-programms\\PCA_index_subbasket_selection\\DAX_subbasket.csv",sep="")
...
par (new=T)
plot ( HistBasketPrices[1,],type="l",col="blue",ylim=c(MinY,MaxY),ylab="",xlab="T")
