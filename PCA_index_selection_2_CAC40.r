...
filenameAEXBasket <- paste("K:\\RANDD\\99-Misc\\Bastiaan\\R-programms\\PCA_index_subbasket_selection\\CAC40_basket.csv",sep="")
...
excludedstock <- length(excludedStocks) - 2                         # location of first excluded stock number
counter <-1
...
for (i in 1:iNumberOfStocks)
{
    ...
    {
        if (excludedstock < length(excludedStocks)) excludedstock <- excludedstock + 1
    }
}
